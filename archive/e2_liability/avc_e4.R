...
## ================================================================================================================
##                                DATA ANALYSIS | AV responsibility STUDY | EXPERIMENT 3               
## ================================================================================================================
## clear workspace
rm(list = ls()) 
...
for(i in 1:dim(d)[1]) {
  pref1 <- d[i,24:25][!is.na(d[i,24:25])] # for a given row, get only the non-NA values
  d_subset[i,2] <- as.numeric(pref1[pref1!= ""]) # and only the non-empty values
  resp1 <- d[i,26:27][!is.na(d[i,26:27])]
  d_subset[i,3] <- as.numeric(resp1[resp1!= ""])
  resp2 <- d[i,28:29][!is.na(d[i,28:29])]
  d_subset[i,4] <- as.numeric(resp2[resp2!= ""])
  liab1 <- d[i,30:31][!is.na(d[i,30:31])]
  d_subset[i,5] <- as.numeric(liab1[liab1!= ""])
  liab2 <- d[i,32:33][!is.na(d[i,32:33])]
  d_subset[i,6] <- as.numeric(liab2[liab2!= ""])
  d_subset[i,1] <- d[i,48][!is.na(d[i,48])] 
}
